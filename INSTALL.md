...
For background route monitoring, use an installed native or development build, scan a real route, and select **Start background route**. Allow both foreground and background location access. The app stores one route session on the device for up to four hours and registers a dedicated Expo location task. Android shows a foreground-service notification; iOS shows its background location indicator. The task owns risk refresh and notifications while enabled, and the screen reads its saved location, risk updates, warning, and latest alert. Reopening Route Guard restores an active session. **Stop background route**, **Stop live position**, or starting another route disables the saved session and stops the OS task. An expired session stops on the next task wake or screen check.

Background timing and locked-screen delivery depend on OS permissions, battery policies, connectivity, and device behaviour. Force-quitting can prevent task delivery. Expo Go is not a substitute for testing this path in a native build. Real-device lock/unlock, app restart, permission revocation, expiry, and stop-during-refresh testing remains required before treating background Route Guard as production-ready.
