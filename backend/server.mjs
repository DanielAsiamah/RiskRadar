...
function buildCrimeDatasetSummary(options = {}) {
  return {
    mode: CRIME_SOURCE_MODE,
    fallbackToApi: CRIME_SOURCE_FALLBACK_TO_API,
    dataRoot: CRIME_DATA_ROOT,
    summary: crimeFileSource.summarizeDataset(options),
  };
}
...
const server = http.createServer(async (request, response) => {
  ...
  if (request.method === 'GET' && url.pathname === '/api/crime-dataset-summary') {
    try {
  ...
  if (request.method === 'GET' && url.pathname === '/api/subscription-status') {
    ...
    const dateKey = String(url.searchParams.get('dateKey') || getUsageDateKey()).trim();
    sendJson(request, response, 200, {
      ...buildUsageStatus({ used, dateKey, entitlement }),
      price: {
        currency: 'GBP',
        monthly: PRO_PRICE_GBP_MONTHLY,
        label: `£${PRO_PRICE_GBP_MONTHLY}/month`,
      },
      freeDailyCheckLimit: FREE_DAILY_CHECK_LIMIT,
