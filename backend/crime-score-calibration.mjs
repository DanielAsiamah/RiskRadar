...
async function main() {
  ...
  const url = new URL(`${BASE_URL}/api/crime-score-calibration`);
  ...
  try {
    ...
    throw new Error(`Crime score calibration returned invalid JSON.\n${text}`);
  }

  if (!response.ok) {
    throw new Error(json?.error || `Crime score calibration failed with ${response.status}.`);
  }
