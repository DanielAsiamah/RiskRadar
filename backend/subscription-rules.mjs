...
export function getUsageDateKey(date = new Date()) {
  ...
  if (Number.isNaN(value.getTime())) return getUsageDateKey(new Date());
  return value.toISOString().slice(0, 10);
}

export function buildUsageStatus({ used = 0, dateKey = getUsageDateKey(), entitlement = 'free' } = {}) {
  ...
  const remaining = Math.max(0, FREE_DAILY_CHECK_LIMIT - normalizedUsed);
  return {
    entitlement: 'free',
    dateKey,
    used: normalizedUsed,
    limit: FREE_DAILY_CHECK_LIMIT,
    remaining,
